...
find . -type f | xargs perl -p -i -e 's|%{_bindir}/perl|%{_bindir}/perl|g;s|#!/QOpenSys/usr/bin/sh|#!/QOpenSys/usr/bin/sh|g'
...
cmake -LAH  -DCMAKE_INSTALL_PREFIX="%{_prefix}" \
    ...
    -DPLUGIN_ARIA=STATIC \
    -DPLUGIN_AUDIT_NULL=AUTO \
    -DPLUGIN_AUTH_0X0100=AUTO \
    -DPLUGIN_AUTH_ED25519=AUTO \
    -DPLUGIN_AUTH_PAM=NO \
    -DPLUGIN_AUTH_TEST_PLUGIN=AUTO \
    -DPLUGIN_BLACKHOLE=AUTO \
    -DPLUGIN_CLIENT_ED25519=AUTO \
    -DPLUGIN_CONNECT=AUTO \
    -DPLUGIN_DAEMON_EXAMPLE=AUTO \
    -DPLUGIN_DEBUG_KEY_MANAGEMENT=STATIC \
    -DPLUGIN_DIALOG_EXAMPLES=AUTO \
    -DPLUGIN_EXAMPLE=AUTO \
    -DPLUGIN_EXAMPLE_KEY_MANAGEMENT=STATIC \
    -DPLUGIN_FEDERATED=NO \
    -DPLUGIN_FEDERATEDX=NO \
    -DPLUGIN_FEEDBACK=AUTO \
    -DPLUGIN_FILE_KEY_MANAGEMENT=STATIC \
    -DPLUGIN_FTEXAMPLE=AUTO \
    -DPLUGIN_HANDLERSOCKET=AUTO \
    -DPLUGIN_INNOBASE=AUTO \
    -DPLUGIN_LOCALES=AUTO \
    -DPLUGIN_METADATA_LOCK_INFO=AUTO \
    -DPLUGIN_PARTITION=AUTO \
    -DPLUGIN_PERFSCHEMA=AUTO \
    -DPLUGIN_QA_AUTH_CLIENT=AUTO \
    -DPLUGIN_QA_AUTH_INTERFACE=AUTO \
    -DPLUGIN_QA_AUTH_SERVER=AUTO \
    -DPLUGIN_QUERY_CACHE_INFO=AUTO \
    -DPLUGIN_QUERY_RESPONSE_TIME=AUTO \
    -DPLUGIN_SEQUENCE=AUTO \
    -DPLUGIN_SERVER_AUDIT=AUTO \
    -DPLUGIN_SIMPLE_PASSWORD_CHECK=AUTO \
    -DPLUGIN_SPHINX=AUTO \
    -DPLUGIN_SPIDER=AUTO \
    -DPLUGIN_SQL_ERRLOG=AUTO \
    -DPLUGIN_TEST_SQL_DISCOVERY=AUTO \
    -DPLUGIN_TEST_VERSIONING=AUTO \
    -DPLUGIN_USER_VARIABLES=AUTO \
    -DPLUGIN_WSREP_INFO=AUTO \
